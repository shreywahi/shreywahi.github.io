...
const LoadingScreen = ({ onLoadLocal, onLoadDrive }) => {
  ...
  const isDevelopment = window.location.hostname === 'localhost' || 
                       ...
                       window.location.hostname.includes('local');useEffect(() => {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-950 via-slate-900 to-gray-950">
      <div className="text-center max-w-md mx-auto px-6">
        ...
        <h2 className="text-2xl font-semibold text-white mb-2">
          ...
          <p className="text-blue-200 mb-8">
            {isDevelopment 
              ...
              : 'Preparing to load latest content...'
            }
          </p>
        ) : driveLoadStarted ? (
          <p className="text-blue-200 mb-8">
            {isDevelopment 
              ? 'Attempting to load latest content (may fall back to local in dev)...' 
              : 'Loading latest content from Google Drive...'
            }
          </p>
        ) : (
          <p className="text-blue-200 mb-8">
        ...
        <button
          ...
          className={`mb-6 px-6 py-3 text-white rounded-lg transition-colors duration-200 border 
          ...
          {manualLoadTriggered ? 'Loading Local Content...' : (
            isDevelopment ? 'Load from Local Files (Dev Mode)' : 'Load from Local Files'
          )}
        ...
        {/* Show loading message when manual load is triggered */}
        {manualLoadTriggered && (
          <div className="text-center animate-fade-in">
            <p className="text-green-300 text-sm">
